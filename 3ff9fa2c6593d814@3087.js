function _1(md){return(
  md`<div style="color: grey; font: 13px/25.5px var(--sans-serif); text-transform: uppercase;"><h1 style="display: none;">Mon Graphique</h1><a href="https://d3js.org/">D3</a> › <a href="/@d3/gallery">Gallery</a></div>
  ...
  Array.from(d3.rollup(data, ([d]) => d.value, d => +d.date, d => d.name))
    ...
    .filter(([date]) => date.getFullYear() >= 2016 && date.getFullYear() <= 2023) // Filtrage des années entre 2016 et 2023
...
{
  const scale = d3.scaleOrdinal(["#FF5733", "#33FF57", "#3357FF"]);// couleurs pour les changers
...
d3.scaleLinear([0, 2], [marginLeft, width - marginRight])
)}
